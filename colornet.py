...
class LowLevelFeatNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 64, kernel_size=3, stride=2, padding=1)  #Number of channels have been changed
...
class LowLevelVideoFeatNet(nn.Module):
    def __init__(self):
        super(LowLevelVideoFeatNet, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1)  #Number of channels have been changed
        self.bn1 = nn.BatchNorm2d(64)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)
        self.bn2 = nn.BatchNorm2d(128)
        self.conv3 = nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1)
...
class GlobalFeatNet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(192*192/2, 1024)
...
class VideoNet(nn.Module):
    def __init__(self):
        ...
        self.low_lv_Video_feat_net = LowLevelVideoFeatNet()
        self.mid_lv_feat_net = MidLevelFeatNet()
        self.global_feat_net = GlobalFeatNet()
        self.class_net = ClassificationNet()
        self.upsample_col_net = ColorizationNet()
    ...
    def forward(self, x1, x2):
        x1, x2 = self.low_lv_Video_feat_net(x1, x2)
        #print('after low_lv, mid_input is:{}, global_input is:{}'.format(x1.size(), x2.size()))
        x1 = self.mid_lv_feat_net(x1)
        #print('after mid_lv, mid2fusion_input is:{}'.format(x1.size()))
        class_input, x2 = self.global_feat_net(x2)
